...
class AddTests(unittest.TestCase):
    ...
    def test_06_add_mysql_to_mysql_dest_table_exists_different_schema(self):
        ...
        """
        try:
            destination_records_start_state = self.mysql_db_lib.get_records_from_table(self.test_database_1, self.test_table_4)
            output = subprocess.check_output([self.dbshadow_executable, '-a', '--source', self.test_table_1, '--dest', self.test_table_4, '--srcConfig', self.mysql_in_config_1, '--destConfig', self.mysql_out_config_1])
            expected_output = "Committed 3 records"
            self.assertTrue(expected_output in output, "Expected the output to contain the text: '{}' but instead this was the output: {}".format(expected_output, output))
            source_records = self.mysql_db_lib.get_records_from_table(self.test_database_1, self.test_table_1)
    ...
    def test_07_add_mysql_to_mysql_happy_path_different_db(self):
        ...
        """
        try:
            output = subprocess.check_output([self.dbshadow_executable, '-a', '--source', self.test_table_1, '--dest', self.test_table_5, '--srcConfig', self.mysql_in_config_1, '--destConfig', self.mysql_out_config_2])
            expected_output = "Committed 3 records"
            self.assertTrue(expected_output in output, "Expected the output to contain the text: '{}' but instead this was the output: {}".format(expected_output, output))
            source_records = self.mysql_db_lib.get_records_from_table(self.test_database_1, self.test_table_1)
